...
async function confirmSubscription(slug: string, token: string): Promise<{ ok: boolean; message: string }> {
  try {
    const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000';
    const res = await fetch(`${apiUrl}/api/v1/public/status-pages/${slug}/confirm/${token}`, {
      cache: 'no-store',
    });
...
export default async function ConfirmPage({
  ...
  const result = await confirmSubscription(slug, token);
      ...
      <div className="max-w-md mx-auto text-center space-y-4 p-8">
        {result.ok ? (
          <>
            ...
            <h1 className="text-xl font-bold text-foreground">Subscription Confirmed</h1>
